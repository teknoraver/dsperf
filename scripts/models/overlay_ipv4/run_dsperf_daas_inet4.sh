...
FILE_NAME="$3_dperf-daas-inet4.csv"
##########################################
...
#for i in $(seq 1 $SAMPLES); do
  if [ ! -f "$OUTPUT_FOLDER/$FILE_NAME" ]; then
    ...
    dsperf -s=$DIN_DEST --daas="$SCRIPT_BASE/models/case_1/daas_inet4.ini" --blocksize=$BYTES_TOTALI -y 1-n $SAMPLES | sed 's/\./,/g' >> "$SCRIPT_BASE/$OUTPUT_FOLDER/$FILE_NAME"
  else
    dsperf -s=$DIN_DEST --daas="$SCRIPT_BASE/models/case_1/daas_inet4.ini" --blocksize=$BYTES_TOTALI -y 0 -n $SAMPLES | sed 's/\./,/g' >> "$SCRIPT_BASE/$OUTPUT_FOLDER/$FILE_NAME"
  fi
#done