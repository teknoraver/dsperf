...
#for i in $(seq 1 $SAMPLES); do
  if [ ! -f "$OUTPUT_FOLDER/$FILE_NAME" ]; then
    ...
    dperf -s=$DIN_DEST --daas="$SCRIPT_BASE/models/case_1/daas_inet4.ini" --blocksize=$BYTES_TOTALI -y 1-n $SAMPLES | sed 's/\./,/g' >> "$SCRIPT_BASE/$OUTPUT_FOLDER/$FILE_NAME"
  else
    dperf -s=$DIN_DEST --daas="$SCRIPT_BASE/models/case_1/daas_inet4.ini" --blocksize=$BYTES_TOTALI -y 0 -n $SAMPLES | sed 's/\./,/g' >> "$SCRIPT_BASE/$OUTPUT_FOLDER/$FILE_NAME"
  fi
#done